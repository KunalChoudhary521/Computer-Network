clear all; clc;
%plot backlog, waiting time, # of packets discarded as function of time for N=5
...
schdfile = 'ps_ex1-3-N5.txt';
sinkfile = 'TSinkOut_ex1-3-N5.txt';
...
title('Discarded Packets for N = 5');
xlabel('Time (microseconds)');
ylabel('# of Packets');
ylim([0  max(cumsum(discarded_packets))*1.2]);
xlim([0  max(time_depart)*1.02]);
