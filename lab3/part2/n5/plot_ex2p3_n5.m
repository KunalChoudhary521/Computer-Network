...
schdfile = 'ps.txt';
sinkfile = 'TSinkPos.txt';

[elapsed_time, packet_size, backlog_pos, ~,priority] = textread(schdfile, '%f %f %f %f %f');

remove_idx = find(priority == 2);%remove data related to movietrace
elapsed_time(remove_idx) = [];
packet_size(remove_idx) = [];
backlog_pos(remove_idx) = [];
...
subplot(3,1,1);
plot(time_depart,backlog_pos);
title('Poisson''s Backlog for N = 5');
xlabel('Time (microseconds)');
ylabel('Backlog (bytes)');
xlim([-5e4 6e6]);
ylim([-0.1e4 max(backlog_pos)*1.2]);
%%%%%%%%%%%%%%%%%%%%%%%%%%Waiting Time%%%%%%%%%%%%%%%%%%%%%%
...
while i <= size(arrival_time,1)
    waiting_time(i) = time_arrival(i) - time_depart(i);
    i = i + 1;
end

subplot(3,1,2);
plot(time_arrival,waiting_time);
title('Poisson''s Waiting Time for N = 5');
xlabel('Time (microseconds)');
...
while  i <= size(packet_size,1)
    if((backlog_pos(i) + packet_size(i)) > 102400)
...
title('Poisson''s Discarded Packets for N = 5');
xlabel('Time (microseconds)');
ylabel('# of Packets');
ylim([0  max(cumsum(discarded_packets))*1.2]);
