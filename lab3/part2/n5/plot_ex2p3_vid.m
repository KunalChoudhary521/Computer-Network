...
title('Video''s Backlog for N = 5');
xlabel('Time (microseconds)');
ylabel('Backlog (bytes)');
xlim([-0.05e6 12e6]);
ylim([-1e4 max(backlog_vid)*1.2]);
%%%%%%%%%%%%%%%%%%%%%%%%%%Waiting Time%%%%%%%%%%%%%%%%%%%%%%
...
while i <= size(arrival_time,1)
    waiting_time(i) = time_depart(i) -time_arrival(i);
    i = i + 1;
end

subplot(3,1,2);
plot(time_arrival,waiting_time);
title('Video''s Waiting Time for N = 5');
xlabel('Time (microseconds)');
...
title('Video''s Discarded Packets for N = 5');
xlabel('Time (microseconds)');
ylabel('# of Packets');
xlim([-0.1e6 10e6]);
