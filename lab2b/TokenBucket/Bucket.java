...
public class Bucket implements Runnable
{
	...
	private void updateNoTokens()
	{
		...
		long difference = (currTime - lastTime); // in nanoseconds

		lastTime = currTime - difference%tokenInterval;

		noTokens += difference/tokenInterval;
		if(noTokens > size){
			noTokens = size;
		}
	}
	...
	public synchronized long getWaitingTime(int tokensToWaitFor)
	{
		...
		long timeNeeded = (tokensToWaitFor - noTokens)*tokenInterval;
